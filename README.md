...
```bash
git clone https://github.com/Eshwarmanupati/Live-Attendence-Platform.git
cd Live-Attendence-Platform
```
...
The frontend is a static bundle; the API needs a host that supports long-lived WebSocket
connections (Render, Railway and Fly all do — Vercel's serverless functions do not).

**1. Database** — create a free MongoDB Atlas cluster, add a database user, and allow network
access. Take the connection string, including a database name:
`mongodb+srv://USER:PASSWORD@cluster.mongodb.net/live-attendance`

**2. API** — deploy `live-attendance-system/` with these environment variables:
...
Build `npm install`, start `npm start`, health check `/health`. A `render.yaml` blueprint is
included. Seed the demo data once with `npm run seed`.

**3. Frontend** — deploy `attendance-frontend/` to Vercel (root directory `attendance-frontend`)
with:
...
`vercel.json` already rewrites every path to `index.html`, so deep links like `/teacher/dashboard`
resolve instead of 404ing.

**4.** Set `CLIENT_URL` on the API to the deployed frontend origin and redeploy. Without it the
